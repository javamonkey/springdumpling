...
public class JMSNotifyWaitProvider implements NotifyWaitProvider
{
	...
	@Override
	public void notify(JoinPoint joinPoint, Object returnValue, final RemoteNotify notify)
	{
		...
		bowl.getPool().execute(new Runnable() {
			public void run()
			{
				...
				queueSender.sendObject(jmsConf.getUrl(), path, jmsConf.getUserName(), jmsConf.getPassword(),
						notify.persisit(), realArgs);
			}
		});
	...
	public void close()
	{
		for (QueueListener l : listenerMap.values())
		{
			l.close();
		}
	}
	...
	@Override
	public void notify(final Object[] args, final String path, final boolean persist)
	{
		bowl.getPool().execute(new Runnable() {
			public void run()
			{
				QueueSender queueSender = new QueueSender();
				queueSender.sendObject(jmsConf.getUrl(), path, jmsConf.getUserName(), jmsConf.getPassword(), persist,
						args);
			}
		});
